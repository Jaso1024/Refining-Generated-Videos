...
if __name__ == "__main__":
    for class_name in ucf101_prompts.keys(): 
        ...
        model.load_state_dict(torch.load('Weights/UnetV2BestWeights_sigmoid.pth'), strict=False)
        ...
        for epoch in range(num_epochs):
            if epoch % 6 == 0 and epoch != 0:
                model.load_state_dict(torch.load(f'Weights/UnetV2BestWeights_CC_{class_name}_TensorMasking.pth'))
            model.train()
            epoch_loss = 0
            with tqdm(total=num_samples, desc=f'Epoch {epoch+1}/{num_epochs}') as pbar:
                for i, (batch_data, batch_labels, inverse_batch_masks, class_names) in enumerate(datagen):
                        ...
                        loss = criterion(output*inverse_batch_masks*255, batch_labels*inverse_batch_masks*255)*100
                ...
                if epoch_loss/num_samples < best_val_mse:
                    ...
                    torch.save(model.state_dict(), f'Weights/UnetV2BestWeights_CC_{class_name}_TensorMasking.pth')
