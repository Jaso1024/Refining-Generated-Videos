...
from DegrainerUtils import grainify, compute_hue_loss
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load('Weights/Degrainer/Degrainer_64x64_D03d02.pth'))
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(total=num_samples, desc=f'Epoch {epoch+1}/{num_epochs}') as pbar:
            for i, (_, batch_labels, _, _) in enumerate(datagen):
                    ...
                    batch_data = grainify(batch_labels, noise_type=random_noise_type)

                    optimizer.zero_grad()
                    
                    batch_data = model(batch_data)

                    loss = criterion(batch_data, batch_labels)
            ...
            if epoch_loss/num_samples < best_val_mse:
                ...
                torch.save(model.state_dict(), 'Weights/Degrainer/Degrainer_64x64_D03d02.pth')
